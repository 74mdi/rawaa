...
export async function POST(request: NextRequest) {
  try {
    ...
    if (data.action === 'login') {
      ...
      await addLog('Connexion', `Admin connectée: ${email}`, ip)
    ...
    if (data.action === 'logout') {
      ...
      if (token) {
        ...
        if (payload) {
          await addLog('Deconnexion', `Admin deconnectee: ${payload.email}`, ip)
        }
      }
    ...
    if (data.action === 'change-password') {
      ...
      if (!admin) return NextResponse.json({ error: 'Admin non trouve' }, { status: 404 })
      ...
      const ip = request.headers.get('x-forwarded-for') || 'unknown'
      await addLog('Mot de passe change', `Admin ${payload.email} a change son mot de passe`, ip)
